...
if ($uid != '') {
    ...
    $getval = "SELECT * FROM `assigndoctor` WHERE `E_mail`='" . $row['E-mail'] . "'";
    $query1 = $connection->query($getval);
    if ($query1->num_rows > 0) {
        $rows = $query1->fetch_assoc();
    }

    if (isset($_POST['sndsubmit'])) {
        $name = $_POST['pname'];
        $doc = $_POST['docname'];
        $date = $_POST['date'];
        $rem = $_POST['msg'];
        $email = $_POST['mail'];
        $sts = '0';
        $insert = "INSERT INTO `reminder` (`patient_name`,`mail`,`doctor_name`,`Date`,`reminder`,`status`) VALUES ('" . ucfirst($name) . "','" . strtolower($email) . "','" . ucfirst($doc) . "','" . $date . "','" . $rem . "','" . $sts . "')";
        //echo $insert;
        //exit;
        if ($connection->query($insert)) {
            $msgsss = 1;
            $suc['msh'] = "Reminder Sended Sucessfully";
            //echo "Reminder Sended Sucessfully";
        } else {
            $msgsss = 0;
            $suc['msh'] = "Reminder Not Sended Sucessfully";
        }
    }
}
?>
    ...
    <body>
                ...
                <div class="container" data-scrollax-parent="true">
                    <div class="row slider-text align-items-end">
                        <div class="col-md-7 col-sm-12 ftco-animate mb-5">
                            ...
                            <h1 class="mb-3" data-scrollax=" properties: { translateY: '70%', opacity: .9}">Send Reminder</h1>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row justify-content-center mb-5 pb-5">
            <form action="" method="POST" enctype="multipart/form-data">  
                <?php if ($_POST['sndsubmit'] == 'Send Reminder') {
                    if($msgsss=0){
                        echo '<span style="color:red;">' . $suc['msh'] . '</span>';
                    }else{
                        echo '<span style="color:green;">' . $suc['msh'] . '</span>';
                    }
                ...
                <input type="text" name ="date" placeholder="date" value ="<?php
                if ($rows['date'] !== '') {
                    echo $rows['date'];
                } else {
                    echo $_POST['date'];
                ...
                <br><input type="text" name ="docname" placeholder="Type" value ="<?php
                if ($rows['doctor_name'] !== '') {
                    echo $rows['doctor_name'];
                } else {
                    echo $_POST['docname'];
                }
                ?>" readonly><br>

                <br><input type="text" name ="msg" placeholder="Type" value ="Dont Forget to attend your appointments" readonly><br>

                <!-- Choose Medicine Profile
                 <br><input type="file" name ="file"><br>-->

                <br><input type="submit" name ="sndsubmit" class="btn btn-primary px-4 py-2" value="Send Reminder">
            </form>
