...
if ($_REQUEST['editid'] !== 0) {
    $selc = "SELECT *FROM `admin` WHERE `id`='" . $_REQUEST['editid'] . "'";
    //echo $selc;
    //exit;
...
if (isset($_POST['editsubmit'])) {
    ...
    if (empty($name) || empty($email) || empty($gender) || empty($mob)) {
        echo "All feilds required";
    } else {
        
        include 'DBConfig.php';
        $edit = "UPDATE `admin` SET `Name`='" . ucfirst($name) . "',`E-mail`='" . strtolower($email) . "',`gender`='" . ucfirst($gender) . "',`Mobile`='" . $mob . "' WHERE `id`='" . $u_id . "'";
//        echo $edit;
//        exit;

        if ($connection->query($edit)) {
            //echo "Updated Sucessfullty";
            header("Location: http://localhost/workphpcode//Hospital/admineditinfo.php");
        } else {
            echo "Not Updated";
        }
    }
    ...
    <body>
                ...
                <div class="container" data-scrollax-parent="true">
                    <div class="row slider-text align-items-end">
                        <div class="col-md-7 col-sm-12 ftco-animate mb-5">
                            ...
                            <h1 class="mb-3" data-scrollax=" properties: { translateY: '70%', opacity: .9}">Edit Admin Details</h1>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row justify-content-center mb-5 pb-5">
            <form action="" method="POST" enctype="multipart/form-data">    
                <br>
                <input type="button" value="Go Back" class="btn btn-primary px-4 py-2" onclick="<a href="admininfo.php?id=<?php echo $_SESSION["adminid"]; ?>">"/>
                <br>
                <br>
                <input type="text" name ="dame" placeholder="Name" value ="<?php
                if ($row['Name'] !== '') {
                    echo $row['Name'];
                } else {
                    echo $_POST['dame'];
