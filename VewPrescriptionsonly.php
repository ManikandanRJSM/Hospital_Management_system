...
error_reporting(0);
session_start();
$status = "UPDATE `prescription` SET `status`='1' WHERE `status`='0' AND `patient_email`='" . $_SESSION['puname'] . "'";
            ...
            <div class="container">
                ...
                <div class="collapse navbar-collapse" id="ftco-nav">
                    <ul class="navbar-nav ml-auto">
                        <li class="nav-item"><a href="patientshome.php" class="nav-link">Home</a></li>
...
<p class="breadcrumbs" data-scrollax=" properties: { translateY: '70%', opacity: 1.6}"><span class="mr-2"><a href="patientshome.php">Home</a></span> <!--<span><a href="admininfo.php?id=<?php echo $_SESSION['pp_id']; ?>">Personal Information</a></span></p>-->

                            <h1 class="mb-3" data-scrollax=" properties: { translateY: '70%', opacity: .9}">Prescriptions</h1>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row justify-content-center mb-5 pb-5">
            <form action="" method="POST" enctype="multipart/form-data">     
                <br>
                <input type="button" value="Go Back" class="btn btn-primary px-4 py-2" onclick="window.location.href='http://localhost/workphpcode//Hospital/patientshome.php'"/>
                <br>

                <table id="prescri" border="4">
                    <br>
                    <thead>
                        <tr>
                           <!-- <th><h5>Check<br>all:</h5><input type="checkbox" name="check_list[]" id ="check_all"></th>-->
                            <th>id</th>
                            <th>Name</th>
                            <th>Date</th>                   
                            <th>Medicine Name</th>
                    ...
                    <tbody>
                        <?php
                        include 'DBConfig.php';
                        $selct = "SELECT * FROM `prescription` WHERE `patient_email`='" . $_SESSION['puname'] . "'";
                        //.//echo  $selct;
                        //exit;
                        $result = $connection->query($selct);
                        $counter = 1;
                        while ($row = $result->fetch_assoc()) {
                            ?>
                            <tr>
                               <!-- <td><input type="checkbox" name="check_list[]" value="<?php echo $row['Username']; ?>"></td>-->
                                ...
                                <td>
                                    <?php
                                    $imagename = $row['med_img'];
                                    $str2arr = explode(",", $imagename);
                                    $count = count($str2arr);
                                    for ($i = 0; $i < $count; $i++) {
                                        $dirname = "prescibtions/";
                                        $images = glob($dirname . $str2arr[$i]);
                                        $imagestr = implode($images); //dbname
                                        echo '<img src="' . $imagestr . '" alt="HTML5 Icon" style="width:110px;height:110px">';
                                    }
                                    ?>
                                </td>
