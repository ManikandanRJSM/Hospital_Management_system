...
if ($uid != '') {
    ...
    //echo $editselect;
    //exit;
    $querys = $connection->query($editselect);
    if ($querys->num_rows > 0) {
        $row = $querys->fetch_assoc();
    }

//    $selectdec = "SELECT * FROM `doctors`";    
//    $queryss = $connection->query($selectdec);
//    if ($queryss->num_rows > 0) {        
//        while($rowa = $queryss->fetch_assoc()){
//      //      print_r($rowa['Name']);
//            
//        }
//    }
    //exit;
//    $editdoc = "SELECT * FROM `doctors`";
//    //echo $editselect;
//    //exit;
//    $query = $connection->query($editdoc);
}
?>
<?php
if (isset($_POST['addsubmit'])) {
    ...
    if (empty($date)) {
        ...
        include 'DBConfig.php';
        $sts = "0";
        $insert = "INSERT INTO `assigndoctor` (`patient_name`,`E_mail`,`Gender`,`type`,`date`,`doctor_name`,`status`) VALUES ('" . ucfirst($name) . "','" . strtolower($mail) . "','" . ucfirst($gender) . "','" . $type . "','" . $date . "','" . $doc . "','" . $sts . "')";
        if ($connection->query($insert)) {
$stsss = 1;
$errror['sux'] = "Doctor Assigned Sucessfully";
            //echo "Doctor Assigned Sucessfully";
        } else {
            // header("Location: http://localhost:/workphpcode//date25/Register.php");
            //echo "<br>";
            $stsss = 0;
$errror['sux'] = "Not Sucessfully";
    ...
    <body>
                ...
                <div class="container" data-scrollax-parent="true">
                    <div class="row slider-text align-items-end">
                        <div class="col-md-7 col-sm-12 ftco-animate mb-5">
                            ...
                            <h1 class="mb-3" data-scrollax=" properties: { translateY: '70%', opacity: .9}">Assign Doctor</h1>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row justify-content-center mb-5 pb-5">
            <form action="" method="POST" enctype="multipart/form-data">   
                <?php
if ($_POST['addsubmit'] == 'Assign Doctor') {
    if ($stsss = 0) {
        echo '<span style="color:red;">' . $errror['sux'] . '</span>';
    } else {
        echo '<span style="color:green;">' . $errror['sux'] . '</span>';
    }
}
?>
...
?>" readonly><br>
                <input type="text" name ="date"  placeholder="Date" value ="<?php echo $_REQUEST['date'] ?>"><?php if ($_POST['addsubmit'] == 'Assign Doctor') {
                echo '<span style="color:red;">' . $valid['val'] . '</span>';}?><br>
                <select name="docname">
                    <?php
                    $selectdec = "SELECT * FROM `doctors`";
                    $queryss = $connection->query($selectdec);
                ...
                <br><input type="submit" name ="addsubmit" class="btn btn-primary px-4 py-2" value="Assign Doctor">
            </form>
